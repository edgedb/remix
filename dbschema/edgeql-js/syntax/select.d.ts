...
export declare type linkDescToSelectElement<L extends LinkDesc> = boolean | TypeSet<anonymizeObject<L["target"]>, cardinalityUtil.assignable<L["cardinality"]>> | (objectTypeToSelectShape<L["target"]> & objectTypeToSelectShape<pointersToObjectType<L["properties"]>> & SelectModifiers) | ((scope: $scopify<L["target"]> & linkDescToLinkProps<L>) => objectTypeToSelectShape<L["target"]> & objectTypeToSelectShape<pointersToObjectType<L["properties"]>> & SelectModifiers);
...
export declare function select<Expr extends ObjectTypeExpression, Shape extends objectTypeToSelectShape<Expr["__element__"]> & SelectModifiers, Modifiers = Pick<Shape, SelectModifierNames>>(expr: Expr, shape: (scope: $scopify<Expr["__element__"]>) => Readonly<Shape>): $expr_Select<{
